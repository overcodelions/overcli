...
<watch_report>
{ "cursor": "10042", "answered": 2, "needs_work": false, "note": "Answered two tester questions." }
</watch_report>`;
    const r = parseWatchReport(text)!;
    expect(r.cursor).toBe('10042');
...
\`\`\`json
{ "cursor": "c1", "answered": 0, "needs_work": true, "note": "needs a fix" }
\`\`\`
</watch_report>`;
    const r = parseWatchReport(text)!;
  ...
  it('drops the placeholder cursor so it does not overwrite a real one', () => {
    const r = parseWatchReport(
      '<watch_report>{"cursor":"(none yet)","answered":0,"needs_work":false,"note":"nothing"}</watch_report>',
    )!;
...
describe('parseWatchReport — detect fields', () => {
  it('parses answer_needed', () => {
    const r = parseWatchReport(
      '<watch_report>{"cursor":"c2","answer_needed":true,"needs_work":false,"note":"new question"}</watch_report>',
    )!;
